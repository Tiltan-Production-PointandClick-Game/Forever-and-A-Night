		...
		public void ShowGUI ()
		{
			...
			if (showSettings)
			{
				...
				if (AdvGame.GetReferences ().settingsManager != null && AdvGame.GetReferences ().settingsManager.inputMethod != InputMethod.TouchScreen)
				{
					...
					if (eventSystem == null)
					{
						disableMouseIfKeyboardControlling = CustomGUILayout.ToggleLeft ("Disable mouse when directly-controlling Unity UI Menus?", disableMouseIfKeyboardControlling, "AC.KickStarter.menuManager.disableMouseIfKeyboardControlling", "If True, then Unity UI mouse input will be disabled when a keyboard-controllable Menu is open");
					}
				}
